class SessionsController < ApplicationController
  ...
  def create
    # #1 search the database for a user with the specified email address in the params hash. We use downcase to normalize the email address since the addresses stored in the database are stored as lowercase strings
  ...
  def destroy
    # #3 delete a user's session. destroy logs the user out by calling destroy_session(current_user), flashes a notice that they've been logged out, and redirects them to root_path

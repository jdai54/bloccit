...
# #17 test that unsigned-in users are redirected to the sign-in page, as they will not be allowed to vote on posts
  context "guest" do
  ...
  # #18 create a context to test signed-in users, who should be allowed to vote on posts
  context "signed in user" do
    ...
    describe "POST up_vote" do
  # #19 expect that the first time a user up votes a post, a new vote is created for the post
  ...
  # #20 test that a new vote is not created if the user repeatedly up votes a post
  ...
  # #21 expect that up voting a post will increase the number of points on the post by one
  ...
  # #22 test to ensure that users are redirected back to the correct view (posts show or topics show) depending on which view they up voted from. We do this by setting request.env["HTTP_REFERER"] to the requesting URL
  ...
  # #23 test to ensure that users are redirected back to the correct view (posts show or topics show) depending on which view they up voted from. We do this by setting request.env["HTTP_REFERER"] to the requesting URL
